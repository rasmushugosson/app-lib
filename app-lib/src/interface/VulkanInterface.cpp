...
void ae::VulkanInterface::CreateDescriptorPool()
{
    ...
    if (vkCreateDescriptorPool(VulkanManager::Get().GetDevice(), &poolInfo, nullptr, &m_DescriptorPool) != VK_SUCCESS)
    {
        AE_THROW_RUNTIME_ERROR("Failed to create ImGui descriptor pool for Vulkan");
    }
}
